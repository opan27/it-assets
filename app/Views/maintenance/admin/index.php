...
<div class="max-w-7xl mx-auto p-6 bg-white rounded-xl shadow">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-bold text-gray-800">Dashboard Maintenance</h2>
        <a href="<?= base_url('maintenance/create') ?>"
           class="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg shadow text-sm font-medium transition">
           + Ajukan Maintenance
        </a>
    </div>
    ...
    <td class="px-4 py-3"><?= esc($t['teknisi_nama'] ?? '-') ?></td>
    ...
    <!-- Tombol Detail -->
        <a href="<?= base_url('maintenance/detail/'.$t['id']) ?>"
    ...
    <!-- Tombol Download PDF -->
    <a href="<?= base_url('maintenance/report/'.$t['id']) ?>" target="_blank"
       class="inline-flex items-center bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-md text-xs font-medium shadow transition">
       PDF
    </a>
</td>
                    ...
                    <?php endforeach; ?>
                <?php else: ?>
                    <!-- ✅ Pastikan tetap 6 kolom, jangan pakai colspan -->
                    <tr class="border-b">
                        <td class="px-4 py-6 text-center text-gray-500">-</td>
                        <td class="px-4 py-6 text-center text-gray-500">-</td>
                        <td class="px-4 py-6 text-center text-gray-500">-</td>
                        <td class="px-4 py-6 text-center text-gray-500">-</td>
                        <td class="px-4 py-6 text-center text-gray-500">-</td>
                        <td class="px-4 py-6 text-center text-gray-500">Belum ada data maintenance</td>
                    </tr>
                <?php endif; ?>
            </tbody>
        </table>
...
<script>
    $(document).ready(function() {
        $('#ticketsTable').DataTable({
            pageLength: 10,
            order: [[0, 'asc']],
            autoWidth: false, // ✅ biar tidak strict
            language: {
                url: '//cdn.datatables.net/plug-ins/1.13.6/i18n/id.json'
            },
            columnDefs: [
                { targets: '_all', defaultContent: '-' } // ✅ fallback kalau ada td kosong
            ]
        });
    });
</script>
