...
class BeritaAcaraPengembalian extends BaseController
{
    protected $picModel;
    ...
    public function index()
    {
        $data['berita'] = $this->picModel
            ->select("
                ...
                berita_acara_pengembalian.filename AS file_name,
                berita_acara_pengembalian.created_at AS dibuat
            ")
            ->join('pegawai', 'pegawai.id = pic_assets.pegawai_id', 'left')
            ->join('assets', 'assets.id = pic_assets.asset_id', 'left')
            ->join('berita_acara_pengembalian', 'berita_acara_pengembalian.pic_asset_id = pic_assets.id', 'left')
            ->where('pic_assets.status', 'released')  // status asset dikembalikan
            ->orderBy('berita_acara_pengembalian.created_at', 'DESC')
            ->findAll();

        return view('berita_acara_pengembalian/index', $data);
    }
    ...
    public function exportDocx($picAssetId)
    {
        ...
        $processor->setValue('jumlah#1', 1);
        $processor->setValue('kondisi#1', $pic['kondisi']);

        $filename = 'berita_acara_pengembalian_'.$pic['id'].'.docx';
        $savePath = WRITEPATH.'exports/'.$filename;
        $processor->saveAs($savePath);

        return $this->response->download($savePath);
    }
}
